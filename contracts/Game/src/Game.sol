...
contract Game is Ownable(msg.sender) {
    uint256 player1Time;
    uint256 player2Time;
    // Game state tracking
    ...
    modifier onlySocket() {
        require(
            msg.sender == socket_address
        );
        _;
    }
    
    constructor() {
     socket_address = msg.sender
    }
    // Initialize game with first player
    function initializeGame(address creator, uint8 _gameType, uint256 _roomNumber) external onlySocket {
    ...
    function joinGame(address player) external onlySocket {
    ...
    function completeGame(address user) external onlySocket {
        if (user == player1) {
    ...
    // Async function to get game details
    function getGameDetails(uint256 _roomNumber) external onlySocket returns (address winner, uint256 timeTaken, uint8 gameTypeResult) {
        ...
        emit GameCompleted(roomNumber, winner);
    ...
    function adminResetGame() external onlyOwner {
        _resetGame();
    }
}